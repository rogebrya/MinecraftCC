...
namespace MinecraftCC {
	public class Map {
		...
		public void FillMap() {
			...
			for (int i = 0; i < XPos; i++) {
				for (int j = 0; j < YPos; j++) {
					int b = rand.Next(Biomes.Count - 2);
					if (rand.Next(101) > 95 && Biomes[b].Name == "Grass") {
						Cells[i, j] = new MapCell(Biomes[b], MapItems[rand.Next(3)]);
					} else {
						Cells[i, j] = new MapCell(Biomes[b], null);
					}
				}
			}
		}
		...
		public void FillLists() {
			Biomes = new List<Biome>() { new Biome("Grass", Brushes.LightGreen), new Biome("Sand", Brushes.Tan), new Biome("Rock", Brushes.Gray),
				new Biome("Water", Brushes.DarkSlateBlue), new Biome("Cliff", Brushes.DarkRed), new Biome("Plateau", Brushes.Brown), new Biome("Wall Plateau", Brushes.Brown)
			};
			...
			field.ReqBiome = new List<Biome>() { (Biomes[0]), (Biomes[5]) };
			field.ScoreMods = new List<(object, int)>() { (mill, 5), (statue, 3), (resort, 3), (field, 2) };
			fisher.ReqBiome = new List<Biome>() { (Biomes[3]) };
			...
			hopField.ReqBiome = new List<Biome>() { (Biomes[0]), (Biomes[1]) };
			...
			jewelry.ReqBiome = new List<Biome>() { (Biomes[4]), (Biomes[6]) };
			...
			plateau.ReqBiome = new List<Biome>() { (Biomes[1]), (Biomes[3]) };
			...
			sandpit.ReqBiome = new List<Biome>() { (Biomes[1]) };
			...
			temple.ReqBiome = new List<Biome>() { (Biomes[5]) };
			...
			wallPlateau.ReqBiome = new List<Biome>() { (Biomes[4]) };
			...
			waterPlateau.ReqBiome = new List<Biome>() { (Biomes[3]) };
	...
	public class MapCell {
		...
		public MapCell(Biome biome, MapItem item) {
			_biome = biome; _item = item;
		}
	...
	public class Biome {
		...
		public Biome(string name, SolidColorBrush color) {
